...
const openaiClient = new OpenAI({ apiKey: process.env.OPENTAI_API_KEY! });
...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "call.session_started") {
    ...
    const realtimeClient = await streamVideo.video.connectOpenAi({
      call,
      openAiApiKey: process.env.OPENTAI_API_KEY!,
    ...
    if (!meetingId) {
      return NextResponse.json({ error: "Miising meetingId" }, { status: 400 });
    }
